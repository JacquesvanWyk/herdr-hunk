...
info="$("$herdr_bin" pane get "$pane_id" 2>/dev/null)" || exit 0
...
sig="$( { git -C "$root" status --porcelain; git -C "$root" diff HEAD 2>/dev/null || git -C "$root" diff; } | shasum | awk '{print $1}')"
state_dir="${HERDR_PLUGIN_STATE_DIR:-$HOME/.cache/herdr-hunk}"
mkdir -p "$state_dir" 2>/dev/null || true
key="$state_dir/$(printf '%s' "$root" | shasum | awk '{print $1}').sig"
[ -f "$key" ] && [ "$(cat "$key" 2>/dev/null)" = "$sig" ] && exit 0
